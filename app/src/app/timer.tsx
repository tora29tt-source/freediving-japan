...
// ─── SUPABASE ────────────────────────────────────────────────────────────────
// training_sessions / training_dives の実スキーマ（sql/supabase_schema.sql）に合わせて保存する。
// RLSが auth.uid() = user_id を要求するため、ログイン中のユーザーでのみ保存できる。
const DISCIPLINE_DB: Record<Discipline, string> = {
  'STA':      'STA',
  'RV STA':   'RVSTA',
  'FRC STA':  'FRCSTA',
};
...
async function saveSession({
  userId, env, discipline, note, dives,
}: {
  ...
  const { data: session, error: sessionErr } = await supabase
    ...
    .insert({ user_id: userId, date: today, env, note: note || null, is_public: false })
    .select('id')
    .single();

  if (sessionErr || !session) {
    return { ok: false, error: sessionErr?.message ?? 'セッションの保存に失敗しました' };
  }

  if (dives.length > 0) {
    const rows = dives.map((d, i) => {
      const parts: string[] = [];
      if (d.fcMs != null) parts.push(`FC ${fmtMs(d.fcMs)}`);
      if (d.minSpo2 != null) parts.push(`Min SpO2 ${d.minSpo2}%`);
      return {
        session_id: session.id,
        user_id: userId,
        dive_number: i + 1,
        discipline: DISCIPLINE_DB[discipline],
        hold_time: Math.round(d.holdMs / 1000),
        note: parts.length ? parts.join(' / ') : null,
      };
    });
    const { error: divesErr } = await supabase.from('training_dives').insert(rows);
    if (divesErr) {
      return { ok: false, error: `ダイブの保存に失敗しました（セッションは保存済み）: ${divesErr.message}` };
    }
  }
...
export default function TimerScreen() {
  ...
  const handleSave = async (note: string, env: Env, discipline: Discipline) => {
    ...
    if (!session?.user) {
      Alert.alert(
        'ログインが必要です',
        'ログを保存するには、マイページからログインしてください。',
      );
      return;
    }

    const result = await saveSession({ userId: session.user.id, env, discipline, note, dives });
    Alert.alert(
      result.ok ? '✓ 保存しました' : 'エラー',
    ...
    <SafeAreaView style={ss.container}>
      ...
      >
        ...
        <View style={ss.controls}>
          ...
          {phase === 'idle' && dives.length > 0 && !session && (
            <Text style={ss.loginHint}>
              ※ 保存にはログインが必要です（
              <Link href="/mypage" style={{ color: C.teal, textDecorationLine: 'underline' }}>マイページ</Link>
              ）
            </Text>
          )}
        </View>
...
const ss = StyleSheet.create({
  ...
  loginHint:  { fontSize: 10, color: C.muted, textAlign: 'center', marginTop: 6 },
