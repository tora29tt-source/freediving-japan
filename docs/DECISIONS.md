...
**背景**：ユーザー報告により3件の不具合を確認。(1)(2)は再現手順から原因を特定して修正、(3)は「録画が保存されない/消える」「1秒程度しか録れていない」という報告を受けてコードレビューで原因候補を特定し修正した。2026-07-14、Chrome MCP（内蔵ブラウザ）で3件とも動作確認済み（詳細は末尾の「検証（2026-07-14）」参照）。**ただし実機（iPhone Safari）での確認はまだ**のため、次回実機使用時に最終確認が必要。

**2026-07-15追記**：(3)についてユーザーが実機（iPhone Safari・最新版）のTABLE/O2/CO2モード（Rest 30秒以上）で再現を報告。上記の対策（`_stopRecording`一元化）が対象としていた「Rest 30秒未満での二重録画競合」とは異なる条件のため、原因調査を再開し別バグを特定・修正した。詳細は[「2026-07-15：STAタイマー録画『1秒しか録れない』再発」](#2026-07-15staタイマー録画1秒しか録れない再発wake-lock未取得が原因)を参照。
...
**検証状況**：2026-07-14、Chrome MCPで動作確認済み（下記「検証（2026-07-14）」参照）。**実機（iPhone Safari）での確認はまだ**。

**実装**：`tools/sta-timer.html`（コミット: "fix: STAタイマー プリセット名にスペース入力不可を修正、1stコントラクション記録をリングタップでも可能に" / "fix: STAタイマー録画のcamRecorder状態不整合を修正"）

### 検証（2026-07-14）

Chrome MCP（内蔵ブラウザ、`http-server`経由でローカル配信）で3件とも実際に操作して確認。

- **(1) プリセット名のスペース入力**：TABLE→セッション作成→「＋プリセットとして保存」の名前入力欄に「Free Dive Test」と入力し、スペース込みで正しく入力・保存できることを確認（「✓プリセット「Free Dive Test」を保存しました」のトースト表示）。
- **(2) リングタップでのFC記録**：Stopwatchモードでホールド中にメインリング（`#timerWrap`）をタップ→「1st: 00:09」としてFC記録され、ホールドは停止せず継続することを確認。記録済み状態でもう一度リングをタップすると正しくHoldが停止しSurface Protocolへ遷移し、ログに「1st 00:09」が記録されることを確認。
- **(3) 録画の途中切れ**：内蔵ブラウザは実カメラへのアクセスが許可されないため（`getUserMedia`が`NotAllowedError`でブロックされる）、`navigator.mediaDevices.getUserMedia`をアニメーションcanvas＋無音オーディオトラックのフェイクストリームに差し替えた上で検証。TABLEモード（Auto／Hold 0:15／**Rest 0:15**＝修正前バグの発生条件だった「30秒未満」／3セット）で連続ホールドを実行した結果、`cameraClips`に3クリップとも保存され、実際の再生時間（`<video>`の`duration`で計測）はSet #1: 31.5秒／Set #2: 32.9秒／Set #3: 31.9秒と、いずれも1秒程度への途切れは発生しなかった。MediaRecorderのエラーや「録画データなし」のトーストも発生せず。セッション確認画面でも#1〜#3すべてに録画アイコンが表示された。
  - **注**：Chromiumベースの内蔵ブラウザ＋モックカメラでの検証であり、iOS Safari実機（バックグラウンド時の`setTimeout`遅延・コーデック対応など、コード内コメントで言及されているSafari固有の懸念）での確認ではない。ロジック（`_stopRecording`による状態管理の一元化）が意図通り動作することは確認できたが、実機での最終確認は別途必要。**→ この直後にユーザーの実機報告により(3)は別原因が発覚。下記参照。**

-----

## 2026-07-15：STAタイマー録画「1秒しか録れない」再発 — Wake Lock未取得が原因

**背景**：2026-07-14の`_stopRecording`一元化修正後、Chrome MCPでは動作確認できていたが、ユーザーがiPhone Safari実機（最新版）のTABLE/O2/CO2モード（Rest 2:00など30秒以上）で試したところ、依然として録画が1秒程度しか保存されない事象を報告。

**前回修正との関係**：2026-07-14の修正は「Rest（休憩）がSurface後の固定録画時間（30秒）より短い場合に、次のHoldが始まる際に新旧2つのMediaRecorderが競合する」レース条件への対策だった。しかし今回の報告はRest 30秒以上（O2 Basicはデフォルト2:00固定）のため、その競合条件には該当しない。**別原因を再調査した。**

**原因**：Wake Lock（画面スリープ防止）の取得漏れ。`requestWakeLock()`が呼ばれるのは以下の2箇所のみだった：
- `beginCountdownOrHold()`内、Breath Upカウントダウンがある場合（`else`分岐）のみ
- `onPause()`のResume時のみ

一方、Hold開始の実処理を行う`startHold()`自体は`requestWakeLock()`を呼んでいなかった。加えて：
- 組み込みプリセット（O2 Basic／CO2 Basic／Relaxation）はすべて「Breath Up: すぐ」（`prepSec=0`）がデフォルトのため、`beginCountdownOrHold()`は常に`else`分岐を通らず`startHold()`へ直行し、Wake Lockが一度も取得されない
- カウントダウンがある設定でも、2セット目以降は`endRest() → startHold()`という別経路で直接Holdが始まり、この経路にもWake Lock取得は無かった

結果として、O2 Basic（Hold Manual・Rest 2:00固定・8セット）のような数十秒〜2分に及ぶHold中にWake Lockが一切保持されず、iPhoneの自動画面ロック（多くは30秒〜1分に設定）でスリープしてしまう状態だった。画面ロック中はcanvas描画ループ（`requestAnimationFrame`）やMediaRecorderのタイムスライス処理が停止・スロットルされるため、「録画が1秒程度しか保存されない」という報告と一致する。2026-07-14の検証でこの問題を見逃していたのは、テスト条件がHold 15秒・Rest 15秒という極端に短い設定で、画面ロックが起きるより先にセッションが終わっていたため。
